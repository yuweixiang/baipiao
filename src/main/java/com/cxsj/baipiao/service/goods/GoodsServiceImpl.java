...
public class GoodsServiceImpl implements GoodsService{
    ...
    public List<Goods> queryGoodsByCategory(Long categoryId, PageRequest request){

        Integer index = request.getPageIndex()*request.getPageSize();
        List<Goods> goodsList = goodsMapper.pageQueryByCatetory(categoryId,index,request.getPageSize());
    ...
    public Goods queryGoodsDetail(Long goodsId){
        ...
        skuList.forEach(sku -> {
            sku.getGoodsSpecList().forEach(goodsSpec->{
                goodsSpec.setSpecValueList(JSONObject.parseArray(goodsSpec.getSpecValues(), String.class));
            });
            sku.setGoodsSpecList(JSONObject.parseArray(sku.getGoodsSpecs(), SkuSpec.class));
        });
